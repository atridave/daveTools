...
class CVG(OpenMayaMPx.MPxNode):
    ...
    def callbackFunc(self,*args):
        print 'callback func'
        loc = cmds.spaceLocator( p=(randint(0,100), randint(0,100),randint(0,100)) )
        cmds.setAttr((loc[0]+'.localScaleX'),10)
        cmds.setAttr((loc[0]+'.localScaleY'),10)
        cmds.setAttr((loc[0]+'.localScaleZ'),10)
